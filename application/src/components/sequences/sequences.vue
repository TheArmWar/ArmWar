...
const { isRecording, allSequences } = defineProps(['isRecording', 'allSequences'])
...
<template>
    <div class="sequences">
        ...
        <div class="sequences-holder">
            <Sequence v-for="sequence in allSequences" :sequenceName="sequence.name" :movements="sequence.movements"
                @delete-sequence="handleDeleteSequence" @play-sequence="handlePlaySequence" />
        </div>
        <div>
            <button class="button" :class="{ recording: isRecording }" @click="handleNewSequence">
