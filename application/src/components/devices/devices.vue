...
<template>
    <div class="devices">
        ...
        <Device v-for="device in allDevices" :name="device.name" :active="device.active" :id="device.id"
            @device-clicked="handleDeviceClicked" />
        <button class="button" @click="handleNewDevice">
            new
        </button>
        <button class="button delete" @click="deleteDevice" v-if="allDevices.length > 0">
            delete a device
        </button>
    </div>
</template>
